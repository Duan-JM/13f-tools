...
class TestDataExporter:
    ...
    def test_export_holdings_to_excel(self, exporter, sample_holdings):
        """测试导出持仓数据到Excel"""
        filepath = exporter.export_holdings_to_excel(sample_holdings, "test_holdings.xlsx")
    ...
    def test_create_summary_report(self, exporter):
        """测试创建多季度汇总报告"""
        # 创建多个季度的示例数据
        holdings_list = []
        for i, quarter in enumerate(["2024Q1", "2024Q2", "2024Q3"], 1):
            holdings = [
                Holding("037833100", "APPLE INC", "COM", 1000000 * i, 150000000.0 * i, 50.0)
            ]
            holdings_data = Holdings(
                cik="0001067983",
                fund_name="BERKSHIRE HATHAWAY INC",
                quarter=quarter,
                period_end_date=datetime(2024, i * 3, 30 if i * 3 != 3 else 31),
                total_value=300000000.0 * i,
                holdings=holdings
            )
            holdings_list.append(holdings_data)
        
        filepath = exporter.create_summary_report(holdings_list, "summary_report.xlsx")
        
        assert os.path.exists(filepath)
        
        with pd.ExcelFile(filepath) as excel_file:
            assert "价值趋势" in excel_file.sheet_names
            assert "历史前10大持仓" in excel_file.sheet_names
            assert "集中度趋势" in excel_file.sheet_names
            
            # 验证价值趋势数据
            trend_df = pd.read_excel(excel_file, sheet_name="价值趋势")
            assert len(trend_df) == 3
            assert trend_df['季度'].tolist() == ["2024Q1", "2024Q2", "2024Q3"]
    ...
    def test_create_summary_report_empty_list(self, exporter):
        """测试空持仓列表创建汇总报告"""
        with pytest.raises(ValueError, match="持仓数据列表不能为空"):
