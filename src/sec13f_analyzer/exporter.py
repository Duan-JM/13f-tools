...
class DataExporter:
    ...
    def export_holdings_changes_to_excel(
        self, holdings_change: HoldingsChange, filename: Optional[str] = None
    ) -> str:
        """
        导出持仓变动数据到Excel文件
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            ...
            if position_changes:
                ...
                position_changes.sort(
                    key=lambda x: abs(x.value_change or 0), reverse=True
                )
            ...
            summary_data = {
                ...
                "数值": [
                    ...
                    len(
                        [
                            c
                            for c in holdings_change.changes
    ...
    def export_to_csv(
        self, data: Union[Holdings, HoldingsChange], filename: Optional[str] = None
    ) -> str:
        """
        导出数据到CSV文件
    ...
    def _calculate_top_n_percentage(self, holdings: Holdings, n: int) -> float:
        ...
        top_n = sorted(holdings.holdings, key=lambda x: x.market_value, reverse=True)[
            :n
        ]
        top_n_value = sum(h.market_value for h in top_n)

        return (
            (top_n_value / holdings.total_value) * 100
