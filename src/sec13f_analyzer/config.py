...
class Config:
    ...
    def is_cache_enabled(self) -> bool:
        """是否启用缓存"""
        return self.config.getboolean('CACHE', 'enable_cache')
    
    def get_cache_ttl(self) -> int:
        """获取缓存有效期（小时）"""
        return self.config.getint('CACHE', 'cache_ttl')
    
    def get_default_export_format(self) -> str:
        """获取默认导出格式"""
        return self.config.get('EXPORT', 'default_format')
    
    def get_output_directory(self) -> str:
        """获取输出目录"""
        return self.config.get('EXPORT', 'output_directory')
    
    def get_default_chart_type(self) -> str:
        """获取默认图表类型"""
        return self.config.get('VISUALIZATION', 'default_chart_type')
    
    def get_chart_style(self) -> str:
        """获取图表样式"""
        return self.config.get('VISUALIZATION', 'chart_style')
    
    def should_show_plots(self) -> bool:
        """是否显示图表"""
        return self.config.getboolean('VISUALIZATION', 'show_plots')
    
    def get_image_format(self) -> str:
        """获取图片保存格式"""
        return self.config.get('VISUALIZATION', 'image_format')
    
    def get_image_dpi(self) -> int:
        """获取图片DPI"""
        return self.config.getint('VISUALIZATION', 'image_dpi')
    ...
    def print_config(self):
        """打印当前配置"""
        print("📋 当前配置:")
        print(f"  • 公司名称: {self.get_company_name()}")
        print(f"  • 联系邮箱: {self.get_email()}")
        print(f"  • User-Agent: {self.get_user_agent()}")
        print(f"  • 请求延迟: {self.get_request_delay()}秒")
        print(f"  • 最大重试: {self.get_max_retries()}次")
        print(f"  • 请求超时: {self.get_timeout()}秒")
        print(f"  • 启用缓存: {'是' if self.is_cache_enabled() else '否'}")
        print(f"  • 默认导出格式: {self.get_default_export_format()}")
